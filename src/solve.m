% To use this function to solve the game with one game theoretical approach
% from the six, the corresponding section must be uncommented and all other
% 5 must be commented
...
function [engineTorque,motorTorque] = solve(payoffEngine, payoffMotor, ...
    ...
    %   ---------------------- PARETO EFFICIENCY AND OPTIMALITY------------
        [paretoStrategies, paretoIndex, ~, ~] = paretoefficiency(payoffEngine, payoffMotor);    
    ...
    %   % if there are more than one Pareto efficient points, then take
    %   % Pareto Optimality with Analogical gates
    ...
    %              
    %         
    %   % take Nash Equilibrium point to be the conflict point
    ...
    %       end          
    %       % KS solution mixed strategies probabilities
    ...
    %       % the mixed strategies engine and motor torque                       
    ...
    %--------------------------------CORE----------------------------------
     ...
     [ payoffWholeCoal, coeffCoal, payoffEngineCoal, payoffMotorCoal, payoffWholeCoalBefore ] = ...
      ...
      % get all non-dominated strategies (Pareto optimality)
      ...
      % take the payoff of all imputations      
      ...
      % find the core from all imputations       
    ...
    %   engineTorque = shapleyEngineTorque;
    %   motorTorque = shapleyMotorTorque;
    %   end
    catch
