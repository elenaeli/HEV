%function [engineTorque,motorTorque] = solvegame(requiredTorque, FuelConsTable, GasEmisTable)
        ...
        front = paretofront(payoffBoth);
        in = find(front);  
        figure
        plot(x,y,'ob');
        hold on
        plot(payoffBoth(in,1), payoffBoth(in,2), 'or');
        ...
        nashEqk = LemkeHowson(-payoffEngine, -payoffMotor, ceil(numStratBoth));
        toc
        ...
        for i = 1 : size(imput,2)
            ...
            [r, c] = ind2sub(size(payoffWholeCoalition), imput(i));
            payoffImput(i,3) = payoffWholeCoalition(r,c);
        end
