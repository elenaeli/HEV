...
\begin{minipage}{.52\textwidth}
	...
	\caption{Serial Hybrid \citep{hybridClassification}}
	\label{fig:serialhybrid}
\end{minipage}%
\begin{minipage}{.52\textwidth}
  	\centering
	\includegraphics[scale=0.59]{figures/parallel}
	\caption{Parallel Hybrid \citep{hybridClassification}}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.4]{figures/nashSolution.png}
...
In general, the Shapley value is about permutations of players and especially which player arrives first or who is able to make a decision first. The order of the players is irrelevant and all various orders have the same probability. Since there are $n!$ permutations of the players, the probability of each is $\frac{1}{n!}$ as shown in Equation \ref{eq:shapley}. Additionally, the probability of player $i$ being at the $k$-th place is $\frac{(k-1)!(n-k)!}{n!}$.
On the one hand, a player who has an influence on the value of the coalition is called a crucial or pivot player. A player $i$ is a pivot player whenever $K$ is a winning coalition and without him the coalition $K-\{i\}$ is a losing coalition. On the other hand, a player is called a dummy player when a coalition $K$ has the same value regardless whether $i$ is participating in it or not, $[v(K) - v(K-\{i\})]=0$. The Shapley value is the last game-theoretical concept.

To conclude, this chapter presented the principles of hybrid vehicles and defined the game to be played formally. In addition, it described the theoretical principles of six game theory solution approaches applicable to cooperative games with two players.